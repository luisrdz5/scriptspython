...
# =============================
# para ejecutar 
#  python3 fotosv4.py new --models "Autoestereos, Bocinas, Tweeters, Frentes, Set de Medios, Video, Servicios"
#
# =============================
load_dotenv()
...
archivo_log = f"logs/log_{datetime.datetime.now().strftime('%Y-%m-%d')}-fotosv4-{short_id}.txt"
    ...
    try:
        with conn.cursor() as cur:
            ...
            if result:
                ...
                cur.execute('INSERT INTO "CatalogoFotos" (sku, model, created_at) VALUES (%s, %s, NOW()) RETURNING id;',
                            (data["sku"], data["model"]))
...
def process_folder(folder_id, model_data, depth=0):
    try:
        for attempt in range(5):
            try:
                ...
                if e.resp.status in [500, 503]:
                    ...
                    write_log(f"⚠️ Error {e.resp.status} en carpeta {folder_id}, reintentando en {wait:.1f}s...")
                    time.sleep(wait)
                else:
                    raise
        else:
            write_log(f"❌ Falló tras múltiples reintentos carpeta {folder_id}")
            return

        for item in items:
            if item["mimeType"] == "application/vnd.google-apps.folder":
                process_folder(item["id"], model_data, depth + 1)
            else:
                file_id = item["id"]
                file_name = item["name"]
                try:
        ...
        write_log(f"❌ Error general procesando carpeta {folder_id}: {e}")
...
def main():
    ...
    parser.add_argument("--models", help="Lista de modelos separados por coma o 'All' para procesar todos", default=None)
    ...
    if args.models:
        if args.models.strip().lower() == "all":
            modelos = models_catalog
            write_log("🌀 Procesando TODOS los modelos del catálogo.")
        else:
            nombres = [m.strip().upper() for m in args.models.split(",")]
            modelos = [m for m in models_catalog if m["name"].upper() in nombres]
            write_log(f"🎯 Procesando modelos específicos: {', '.join(nombres)}")
    else:
        print("\n=== Selecciona el modelo que quieres procesar ===\n")
        for i, m in enumerate(models_catalog, start=1):
            print(f"{i}. {m['name']} → {m['model']}")
        choice = int(input("👉 Ingresa el número del modelo: "))
        modelos = [models_catalog[choice - 1]]

    for model_data in modelos:
        model_folder_id = find_drive_folder_for_model(model_data["name"])
        if not model_folder_id:
            continue
        write_log(f"🚀 Procesando carpeta {model_data['name']}...")
        process_folder(model_folder_id, model_data)
        write_log(f"✅ Terminado {model_data['name']}\n")

    write_log("🎯 Proceso completado con éxito para todos los modelos seleccionados.")
